...
# ---------------------------------------------------------------------------
# Generators for the remaining 45 shops
# ---------------------------------------------------------------------------
SURNAMES = [
...
# (lat, long) base per neighborhood — approximate Mumbai coords, roughly
# north-to-south so distance filtering behaves sensibly.
NEIGHBORHOOD_COORDS = {
...
def _generate_shops(count: int = 45) -> list[dict]:
    rng = random.Random(42)  # fixed seed -> reproducible data
    surnames = rng.sample(SURNAMES, count)
    names = [rng.choice(FIRST_NAMES) for _ in range(count)]
    neighborhoods = [rng.choice(NEIGHBORHOODS) for _ in range(count)]
    ...
    for i in range(count):
        ...
        # Small jitter around the neighbourhood so shops don't stack exactly.
        ...
        suffix = rng.choice(SHOP_SUFFIXES)
...
SHOPS = CURATED + _generate_shops(45)
