...
# 41. Mobile vendors (thela/cart): a vendor with no fixed place is found by
# their stops — place + weekly/daily timing — not by an address.
from datetime import datetime, timedelta

from app import schedule as _sched

# 41a. Schedule maths, against a frozen clock (a Wednesday, 11:00).
...
# 41b. Registering a cart and adding rounds.
res = client.post("/api/shops", json={
...
# 41c. Search points customers at the round they can actually reach, and the
...
# 41d. A cart with no rounds yet still behaves like a shop at its base location.
res = client.post("/api/shops", json={
...
# 41e. Adding a round to a shop registered as fixed flips it to mobile — that's
...
# 41f. Both pages carry the vendor UI.
page = client.get("/shopkeeper").text
