...
assert 'loadSampleData' in panel and 'Load 50 sample places' in panel
client.post("/api/admin/data/clear")
...
assert '<span class="tab-text">Places</span>' in panel
assert '<th>Place</th>' in panel
...
# ---------------------------------------------------------------------------
# 50. Plan a purchase: a free-text goal -> AI shopping list -> nearby shop
# ---------------------------------------------------------------------------

# 50a. _parse_items generalises to any second field, so one parser serves both
# the shelf-photo shape (name/category) and the plan shape (name/note).
assert _ai._parse_items(
    '[{"name":"onion","note":"2-3, chopped"}]', second_key="note"
) == [{"name": "onion", "note": "2-3, chopped"}]
assert _ai._parse_items(
    '```json\n[{"name":"cement","note":""}]\n```', second_key="note"
) == [{"name": "cement", "note": ""}]
assert _ai._parse_items("- Onion | 2-3\n- Cement | ", second_key="note") == [
    {"name": "Onion", "note": "2-3"}, {"name": "Cement", "note": ""}]
print("PASS _parse_items generalises to a configurable second field")

# 50b. plan_items: grounded with a web search, parsed the same way. Both the
# search and the model are faked, so this needs no network and no API key.
_orig_context_block = _ai.web_search.context_block
_orig_call_text = _ai.call_text
_ai.web_search.context_block = lambda *a, **k: ""
_ai.call_text = lambda *a, **k: '[{"name":"cement","note":""},{"name":"trowel","note":""}]'
items, err = _ai.plan_items("build a small wall")
assert items == [{"name": "cement", "note": ""}, {"name": "trowel", "note": ""}], items
assert err == ""
# An empty goal never reaches the model.
assert _ai.plan_items("") == ([], "Kya banana ya karna hai, likho.")
# The model returning nothing usable is reported, not silently empty.
_ai.call_text = lambda *a, **k: ""
empty_items, empty_err = _ai.plan_items("anything")
assert empty_items == [] and empty_err
_ai.call_text = _orig_call_text
_ai.web_search.context_block = _orig_context_block
print("PASS plan_items: grounded prompt, parsed reply, empty-input/empty-reply errors")

# 50c. The /plan endpoint: items generated by the (faked) LLM are matched
# against real nearby shops exactly the way a normal search would — Verma
# Hardware (_hw_id) stocks "Cement", so it's the match; nothing sells
# "unobtainium" so that line comes back with no shop rather than a crash.
_orig_plan_items = _ai.plan_items
_ai.plan_items = lambda *a, **k: (
    [{"name": "cement", "note": "2 bags"}, {"name": "unobtainium", "note": ""}], "")
res = client.get("/api/food/plan", params={
    "q": "build a small wall", "lat": 19.0760, "long": 72.8777})
assert res.status_code == 200, res.text
plan = res.json()
assert plan["query"] == "build a small wall" and plan["error"] == ""
by_name = {it["name"]: it for it in plan["items"]}
assert by_name["cement"]["note"] == "2 bags"
assert by_name["cement"]["shop"]["shop_id"] == _hw_id, by_name["cement"]["shop"]
assert by_name["unobtainium"]["shop"] is None
print("PASS /api/food/plan matches generated items to real nearby shops")

# 50d. Nothing generated at all -> an empty list plus the reason, not a 500.
_ai.plan_items = lambda *a, **k: ([], "No AI model is set up for this yet.")
empty = client.get("/api/food/plan", params={
    "q": "??", "lat": 19.0760, "long": 72.8777}).json()
assert empty["items"] == [] and empty["error"]
_ai.plan_items = _orig_plan_items
print("PASS /api/food/plan surfaces the reason when nothing could be generated")
